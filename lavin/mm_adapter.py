...
class RepAdapter_Router(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.conv_A = nn.Conv1d(in_features,hidden_dim, 1,groups=1, bias=False)
        self.conv_B = nn.Conv1d(hidden_dim, in_features, 1, groups=1, bias=False)

        self.conv_A1 = nn.Conv1d(in_features, hidden_dim, 1, groups=1, bias=False)
        self.conv_B1 = nn.Conv1d(hidden_dim, in_features, 1, groups=1, bias=False)
        ...
        nn.init.xavier_uniform_(self.conv_A1.weight)
    ...
    def forward(self, x, weights=None):
        with autocast():
            if weights is None:
                weights = torch.softmax(self.expert_weights(x)/self.t, -1).half()  # [bs, 128, 2]

            x = x.transpose(1, 2)
            x_ = self.dropout(self.conv_A(x))
            x1_ = self.dropout(self.conv_A1(x))
            
            y_ = self.conv_B(x_) * self.scale  # [bs, 4096, 128]
            y1_ = self.conv_B1(x1_) * self.scale
            ...
            x = y_ * weights[:, :, 0, None] + y1_ * weights[:, :, 1, None] + x
