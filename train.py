...
# from util.datasets import ScienceQADataSet, InstrcutDataSet, VidCmsDataSet
from util.datasets import InstrcutDataSet, VidCmsDataSet
...
def get_args_parser():
    ...
    parser.add_argument('--visual_adapter_type', type=str, default='normal', metavar='LENGTH',choices=['normal','router','router_block', 'no_clip_adpt'],
    ...
    parser.add_argument('--caption_file', type=str, default='/data/yangyp/LaVIN-main/data/captions.json')
    parser.add_argument('--data_root', type=str, default='/data/yangyp/LaVIN-main/data')
    parser.add_argument('--use_caption', action='store_true', help='use image captions or not')
    parser.add_argument('--do_pretrain', action='store_true', help='pre-train on large scale vl instruction')
    parser.add_argument('--video_folder', type=str, default='/data/yangyp/Video-ChatGPT-main/MSRVTT/TrainValVideo')
    parser.add_argument('--cms_type', type=str, default='eff', choices=['eff', 'int', 'att'],
                        help='CMS type: eff (effect), int (intention), att (attribute)')

    parser.add_argument('--lr_qformer', type=float, default=None, metavar='LR', help='learning rate (absolute lr)')
    parser.add_argument('--lr_lora', type=float, default=None, metavar='LR', help='learning rate (absolute lr)')
...
def main(args):
    ...
    if args.do_pretrain:
        ...
        dataset_train = VidCmsDataSet(args, 'train', args.llama_model_path, args.max_seq_len, cms_type=args.cms_type)
    ...
    def param_groups_weight_decay_and_lr(
            model,
            weight_decay
    ):
        decay_lr3 = []
        ...
        for name, param in model.named_parameters():
            ...
            if param.ndim <= 1 or name.endswith(".bias"):
                if 'adapter_attn' in name:
                    no_decay_lr3.append(param)
                else:
                    no_decay_lr4.append(param)
            else:
                if 'adapter_attn' in name:
                    decay_lr3.append(param)
                else:
                    decay_lr4.append(param)

        # print(len(no_decay_lr3))
        # print(len(no_decay_lr4))
        # print(len(decay_lr3))
        # print(len(decay_lr4))
        # input('decay_lr')

        return [
            {'params': no_decay_lr3, 'weight_decay': 0., 'lr': args.lr_lora, 'name_type': 'lora'},
            {'params': no_decay_lr4, 'weight_decay': 0., 'lr': args.lr_qformer, 'name_type': 'qformer'},
            {'params': decay_lr3, 'weight_decay': weight_decay, 'lr': args.lr_lora, 'name_type': 'lora'},
            {'params': decay_lr4, 'weight_decay': weight_decay, 'lr': args.lr_qformer, 'name_type': 'qformer'},
        ]

    param_groups = param_groups_weight_decay_and_lr(model_without_ddp, args.weight_decay)

    # print('len(param_groups)', len(param_groups))
    ...
    optimizer = bnb.optim.AdamW32bit(param_groups, betas=(0.9, 0.95),is_paged=True) #torch.optim.AdamW(param_groups, lr=args.lr, betas=(0.9, 0.95))
    print(optimizer)
