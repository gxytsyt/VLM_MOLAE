...
def load_and_change_llava_statedict(llama_model_path, model_name):
    ...
    for shard_file in resolved_archive_file:
        ...
        del state_dict
    # print(state_dict_all.keys())  # 'model.embed_tokens.weight', 'model.layers.0.self_attn.q_proj.weight'
    # print(len(state_dict_all.keys()))  # 325
    ...
    # for kk in state_dict_new.keys():
    #     print(kk, state_dict_new[kk].shape)
...
llm_model:str,
    ...
    # checkpoint, tokenizer, params = load_and_change_llava_statedict(ckpt_dir, llm_model)
...
video_folder: str,
    ...
    assert cms_type in CMS_CONFIG_EVAL, f"cms_type must be one of {list(CMS_CONFIG_EVAL.keys())}, got {cms_type}"
    cms_cfg = CMS_CONFIG_EVAL[cms_type]
    ...
    print('split: ', split)  # test

    datas_all = json.load(open(os.path.join(data_root, 'vlm_molae_json/all_test_cap.json')))

    random.seed(42)
    random.shuffle(datas_all)
    # datas_all = datas_all[:5000]
    video_path = video_folder
    total_items = len(datas_all)  # 36518
    ...
    preds = []
    # id_eval = 0
    for i in tqdm.tqdm(range(total_items//max_batch_size+1)):
        print('progresses: ', i, ' / ', total_items//max_batch_size+1)
        # batch_qids = qids[i*max_batch_size:(i+1)*max_batch_size]
        batch_items = datas_all[i*max_batch_size:(i+1)*max_batch_size]
        ...
        for item in batch_items:
            ...
            caption_li = item['caption_li']
            cms_li_li = item[cms_cfg['gt_key']]
            indicator = 1

            with open(f"{video_path}/{video_name}", "rb") as f:
            ...
            prompt_answer = []
            for i_ in range(len(caption_li)):
                caption_li_i = caption_li[i_]
                cms_l_i = cms_li_li[i_]
                for cms in cms_l_i:
                    prompt_answer.append(' ' + caption_li_i + ' ' + cms)

            prompts.append(prompt_question)
            answers.append(prompt_answer)
            images.append(video_fea.unsqueeze(0))
            indicators.append(indicator)
        images = torch.cat(images, 0)

        max_gen_len = cms_cfg['max_gen_len']
        results = generator.generate(
            prompts, images=images, indicators=indicators, max_gen_len=max_gen_len, temperature=generation_temperature,
            top_p=top_p, n_feats=n_prompt
        )

        for result in results:
            # print(result)
            pred = result.split('Response:', 1)[1].strip()
            pred = ' ' + ''.join(pred.split(cms_cfg['split_keyword'])).replace('  ', ' ')
    ...
    gth_di = {}
    eval_id = 0
    for i, pre_i in enumerate(preds):
        all_result_li.append({'pred': pre_i, 'gth': answers[i]})
        pre_di[eval_id] = pre_i
        gth_di[eval_id] = answers[i]
        eval_id += 1

    save_folder = adapter_path.rsplit('/', 1)[0]
    epoch_num = adapter_path.rsplit('/', 1)[-1].split('.')[0].split('-')[-1]
    with open(save_folder + '/preds-epoch' + str(epoch_num) + cms_cfg['save_suffix'] + '.json', 'w') as f:
        json.dump(all_result_li, f)

    # evaluate cms
    avg_bleu_score, bleu_scores = Bleu(4).compute_score(gth_di, pre_di)
    print('bleu:', avg_bleu_score)
    avg_cider_score, cider_scores = Cider().compute_score(gth_di, pre_di)
    print('cider:', avg_cider_score)
    avg_rouge_score, rouge_scores = Rouge().compute_score(gth_di, pre_di)
    print('rouge:', avg_rouge_score)
    avg_meteor_score, meteor_scores = Meteor().compute_score(gth_di, pre_di)
