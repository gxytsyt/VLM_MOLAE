...
# cms_type -> (GT键名, split关键词, prompt问题, max_gen_len, 保存后缀)
CMS_CONFIG_CAP = {
    'eff': {
        'gt_key': 'eff_li',
        'split_keyword': 'Effect:',
        'prompt_question': 'What will be the effect?',
        'max_gen_len': 26,
        'save_suffix': '_eff',
    },
    'int': {
        ...
        'prompt_question': 'What is the intention?',
        'max_gen_len': 21,
        'save_suffix': '_int',
    },
    'att': {
        ...
        'prompt_question': 'What is the attribute of the people?',
        'max_gen_len': 8,
        'save_suffix': '_att',
    },
}
...
video_folder: str,
    ...
    assert cms_type in CMS_CONFIG_CAP, f"cms_type must be one of {list(CMS_CONFIG_CAP.keys())}, got {cms_type}"
    cms_cfg = CMS_CONFIG_CAP[cms_type]
    ...
    datas_all = json.load(open(os.path.join(data_root, 'vlm_molae_json/all_test.json')))
    random.seed(42)
    ...
    for i in tqdm.tqdm(range(total_items//max_batch_size+1)):
        ...
        for item in batch_items:
            ...
            prompt_question = 'Context: ' + caption + '. Question: ' + cms_cfg['prompt_question'] + ' Response:'
            prompt_answer = gt_li
        ...
        for result in results:
            pred = result.split(cms_cfg['split_keyword'], 1)[1].strip()
