...
def train_one_epoch(model: torch.nn.Module,
    ...
    metric_logger.add_meter('lr1', misc.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    metric_logger.add_meter('lr2', misc.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    metric_logger.add_meter('lr3', misc.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    metric_logger.add_meter('lr4', misc.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    ...
    # input('len(data_loader)')

    for data_iter_step, (examples, labels, example_mask, video_fea, indicators) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
    # for data_iter_step, (examples, labels, example_mask, images, indicators) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        c_loss = model(examples, labels, video_fea=video_fea, prefix_vid=prefix_video, img_indicators=indicators)
        ...
        metric_logger.update(lr2=optimizer.param_groups[1]["lr"])
        metric_logger.update(lr3=optimizer.param_groups[2]["lr"])
        metric_logger.update(lr4=optimizer.param_groups[3]["lr"])
